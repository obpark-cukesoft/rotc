...
class DbCompanyInfo extends DbConnector
{
	...
	**/
	...
	public function QueryList($page_no, $order)
	{
		...
		$sel = "SELECT m.user_name as mem_name, c.* 
	...
	public function QueryOwner($mem_id)
	{
		...
		$sql = "SELECT m.user_name as mem_name, c.* 
						FROM tbl_member_place AS c 
						JOIN tbl_member_base AS m ON c.mem_id=m.rec_id  ORDER BY company ASC";
		if($mem_id>0)
			$sql = "SELECT m.user_name as mem_name, c.* 
						FROM tbl_member_place AS c 
						JOIN tbl_member_base AS m ON c.mem_id=m.rec_id  WHERE c.mem_id=".$mem_id." ORDER BY company ASC";
		$this->result = $this->Query($sql);
		if(!$this->result)
			return 0;
		return 1;
	...
	public function QueryInfo($rec_id)
	{
		...
		$sql = "SELECT m.user_name as mem_name, c.* 
						FROM tbl_member_place AS c 
						JOIN tbl_member_base AS m ON c.mem_id=m.rec_id  WHERE c.rec_id=".$rec_id;
		$this->result = $this->Query($sql);
		if(!$this->result)
			return 0;
		return 1;
